...
class Env:
    ...
    # 1-selected a completed job; 2-correct operation but wrong robot, robot unavailable -10; 3-selected task, robot does not support processing
        ...
        if self.robot_state[robot_id] == 0:                    #2-工序选择正确，但是机器选择错误，机器不可用-1
            self.reward = -1
            self.state[-1]  = 0
            self.state[-2]  = 1
    ...
    # Calculate processing time
        ...
        task.processing_time = task.ref_time[0]+(task.ref_time[1]-task.ref_time[0])*param # 任务处理时间，待增加干扰项 lirl
        # task.processing_time = task.ref_time[0]+ param # 任务处理时间，待增加干扰项 
